...
- Finish the braille / Floyd–Steinberg article: the three before/after figures are rendered with the shipped engine and font (`docs/images/braille-*.webp`), the fallback advance is re-measured (0.684em → 80 rows for the sample portrait, replacing the reviewer's ~82), and the packer excerpt is restored to the engine's exact lines.
- Record the 2026-09-03 IndexNow POST (13 URLs, HTTP 200) and the `90dy/awesome-ascii#4` link fix.
...
- Draft technical articles and assets under `docs/`.
